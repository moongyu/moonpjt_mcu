...
#define ULTRA_ON_HUMI	(40-10)//(50-10)	// 50% 기준 -> 40% 변경
#define ULTRA_OFF_HUMI	(40+10)//(50+10)
...
#define FAN_ON_TIME		(300)	//(300)	// on time 5분
...
void check_abnormal(void)
{
	...
	if(waterlevel_forced_off == 1)
	{
		OUT_ALRAM = 1; 
	}
	else if((f_temp_result >= 60) || (f_temp_result <= 20))
	{
		OUT_ALRAM = 1; 
		for(i=0; i<60000; i++);
		for(i=0; i<60000; i++);
