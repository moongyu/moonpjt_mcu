...
void Lcd_Float(char, char, unsigned int);
//LCD 명령  쓰기 함수	   
...
void Lcd_Char(char x, char y, char type,unsigned char text)
{
	...
	{
		Lcd_Data((text/10)+0x30);	//숫자일 경우 
		Lcd_Data((text%10)+0x30);
	}
}

void Lcd_Float(char x, char y, unsigned int text)
{
	Lcd_Position(x,y);
	Lcd_Data((text / 1000)+0x30);	//숫자100자리 경우
	Lcd_Data((text % 1000)/100+0x30);	//숫자10자리 경우 
	Lcd_Data((text % 1000)%100/10+0x30); //숫자1자리 
	Lcd_Data('.');
	Lcd_Data(((text % 1000)%100)%10+0x30); //숫자0.1자리 
}
